...
func (ps *PersistStorage) UpdateUserBalance(ctx context.Context, userID int) (dto.Amount, error) {
	var balance dto.Amount
	row := ps.QueryRow(ctx, selectUserBalance, userID)
	if err := row.Scan(&balance.Current, &balance.Withdrawn); err != nil {
		return dto.Amount{}, err
...
func (ps *PersistStorage) UpdateUserWithdrawn(ctx context.Context, userID int) (dto.Amount, error) {
	var balance dto.Amount
	row := ps.QueryRow(ctx, selectUserBalance, userID)
	if err := row.Scan(&balance.Current, &balance.Withdrawn); err != nil {
		return dto.Amount{}, err
