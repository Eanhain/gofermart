...
func (s *Service) GetUserBalance(ctx context.Context, username string) (dto.Amount, error) {
	...
	balance, err := s.balance.GetUserBalance(ctx, id)
	if err != nil {
		return dto.Amount{}, err
	}
	return balance, nil
...
func (s *Service) PostUserWithdrawOrder(ctx context.Context, username string, order dto.Withdrawn) error {
	...
	userID, err := s.auth.GetUserID(ctx, username)
	if err != nil {
		return err
	}
	...
	if err := s.orders.InsertNewUserOrder(ctx, order.Order, userID, "NEW", 0); err != nil {
		return err
	}
	amount, err := s.balance.GetUserBalance(ctx, userID)
	if err != nil {
		return err
	}
	...
	if err := s.balance.UpdateUserBalance(ctx, userID, order.Sum*-1); err != nil {
		s.log.Warnln("can't update user balance", username, amount, order)
		return err
	}
	if err := s.balance.UpdateUserWithdrawn(ctx, userID, order.Sum); err != nil {
		s.log.Warnln("can't update user withdrawn", username, amount, order)
		return err
	}
	if err := s.orders.InsertOrderWithdrawn(ctx, userID, order); err != nil {
